...
% (1st coluumn is time, 2nd colum is vehicle speeed)
% 載入行程 FTP-75
...
drag_coefficient = 0.3;  % Drog coefficient 車身牽引力
...
% Fuel cell car paraments 燃料電池參數(簡易)
fuel_cell_efficiency = 0.6;  % Efficiency of the fuel cell 轉換效率
...
% Calculate power requires at wheels 輪胎驅動功率
wheel_power = tractive_force .* (vehicle_speed/3.6);  % Power in watts

% Fuel cell car calculations 燃料電池計算
fc_power_input = wheel_power / fuel_cell_efficiency;  % 輸出功率
fc_electrical_losses = fc_power_input * electrical_losses;  % 電損
fc_required = fc_power_input .* fc_electrical_losses;  % 總消耗功率
fc_hydrogen_consumption = fc_required / (120 * 1e3);  % hydrogen consumption in kg/s (120 MJ/kg)H2消耗
fc_efficiency = wheel_power / fc_required;  % 效率=輸出/總消耗功率

% Fuel cell car calculations 氫引擎計算
he_power_input = wheel_power / engine_efficiency;  % 輸出功率
...
he_hydrogen_consumption = he_required / (120 * 1e3);  % hydrogen consumption in kg/s (120 MJ/kg)H2消耗
...
% Plot Consumption 繪製耗氫曲線圖
subplot(2, 2, 1);
plot(time , fc_hydrogen_consumption, '-b', 'DisplayName', 'Fuel Cell Car Hydrogen Consumption');
hold on;
plot(time , he_hydrogen_consumption, '-r', 'DisplayName', 'Hydrogen Engine Car Hydrogen Consumption');
xlabel('Time (s)');
...
xlabel('Time (s)');
ylabel('Cumulative Hydrogen Consumption');
